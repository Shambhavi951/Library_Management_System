...
export const schemas = {
  publication: Joi.object({
    ...
    publication_year: Joi.number().integer().allow(null),
  ...
  copy: Joi.object({
    ...
    copy_number: Joi.number().required(),
  ...
  member: Joi.object({
    ...
    preferred_branch_id: Joi.number().integer().required(),
...
export const membersList = asyncHandler(async (req, res) => {
  const rows = await query(
    `SELECT m.member_id, m.first_name, m.last_name, m.email, m.phone_number, m.home_branch_id, hb.branch_name AS home_branch_name, 
            m.preferred_branch_id, pb.branch_name AS preferred_branch_name, m.membership_plan_id, mp.plan_name, 
     ...
     LEFT JOIN branches pb ON pb.branch_id = m.preferred_branch_id
...
export const createMember = asyncHandler(async (req, res) => {
  ...
  const rows = await query(
     ...
     WHERE m.member_id = (SELECT id FROM @memberIds);`,
    {
      ...
      branchId: req.body.home_branch_id,
  ...
  await query(
    `UPDATE members 
     SET first_name = @firstName, last_name = @lastName, email = @email, phone_number = @phone, 
         home_branch_id = @homeBranchId, preferred_branch_id = @prefBranchId, membership_plan_id = @planId
     WHERE member_id = @memberId`,
    {
      ...
      prefBranchId: req.body.preferred_branch_id,
      planId: plan.membership_plan_id,
      memberId
    }
  );
  ...
  if (hash) {
    await query(
      `UPDATE user_accounts 
       SET username = @email, email = @email, password_hash = @hash, password_salt = @salt, branch_id = @homeBranchId
       WHERE member_id = @memberId`,
      { email: req.body.email, hash, salt, homeBranchId: req.body.home_branch_id, memberId }
    );
  } else {
    await query(
      `UPDATE user_accounts 
       SET username = @email, email = @email, branch_id = @homeBranchId
       WHERE member_id = @memberId`,
      { email: req.body.email, homeBranchId: req.body.home_branch_id, memberId }
    );
  }
